...
#else
    HPX_CXX_CORE_EXPORT template <typename Action>
    [[nodiscard]] hpx::tracing::annotation_handle const& get_action_name_tracing() noexcept
    {
        static auto sh = hpx::tracing::create_annotation_handle(get_action_name<Action>());
        return sh;
    }
#endif
}    // namespace hpx::actions::detail
